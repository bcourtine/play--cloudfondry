...
public class CloudFoundryDBPlugin extends PlayPlugin {
    ...
    private void rdbmsServiceConfig(Properties p) {
        ...
        RdbmsServiceInfo rdbmsServiceInfo = rdbmsServices.get(0);
        if (rdbmsServiceInfo == null) {
            return;
        }

        if (rdbmsServiceInfo.getLabel() != null && rdbmsServiceInfo.getLabel().startsWith("postgres")) {
    ...
    private void morphiaPluginConfig(Properties p, MongoServiceInfo mongoServiceInfo) {
        ...
        // id type configuration to "ObjectId" (default), if not defined (canalso be "Long").
